...
public class VirtualEnergyRegistry extends WorldSavedData {
    ...
    public VirtualEnergyRegistry(String name) {
        super(name);
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("Private")) {
            ...
            for (String uuidString : privateContainerUUIDs.getKeySet()) {
                UUID uuid = UUID.fromString(uuidString);
                NBTTagCompound privateContainers = privateContainerUUIDs.getCompoundTag(uuidString);
    ...
    public static class VirtualEnergyContainer implements IWirelessEnergyContainer, INBTSerializable<NBTTagCompound> {
        ...
        @Override
        public long changeEnergy(long long_value) {
            AtomicLong temp = new AtomicLong(totalWirelessEnergy.get());
            temp.addAndGet(long_value);

            if (temp.get() < totalWirelessEnergy.get() && long_value > 0) {
                totalWirelessEnergy.set(Long.MAX_VALUE);
                return Long.MAX_VALUE;
            } else
                return totalWirelessEnergy.addAndGet(long_value);
        }
